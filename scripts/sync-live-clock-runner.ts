...
async function buildStrictLiveFlashUrls() {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    throw new Error("Missing Supabase env vars for live clock runner");
  }
  ...
  const { data: rawMatches, error: matchesError } = await supabase
    .from("matches")
    .select("season_id,source_url,status")
    .in("status", ["LIVE", "HT"])
    .limit(80);

  if (matchesError) throw matchesError;

  const candidates = (rawMatches as LiveMatchRow[] | null) ?? [];
...
async function main() {
  const activeInputs = await buildStrictLiveFlashUrls();

  if (activeInputs.length === 0) {
    console.log("sync:live:clock -> no matches currently LIVE/HT. Skipping fast.");
    return;
  }
  ...
  console.log(
    "sync:live:clock -> active live competitions:",
